/**
 * Browser-side EM weekly plan Excel parser (mirrors backend/lib/weeklyPlanExcel.js).
 */
...
function isWorkUpdateHeaderCol(matrix, colIdx) {
  if (!Number.isFinite(colIdx) || colIdx < 0) return false;
  for (let r = 0; r < Math.min(Array.isArray(matrix) ? matrix.length : 0, 15); r += 1) {
    const t = cellText(matrix[r]?.[colIdx])
      .replace(/\s+/g, " ")
      .trim()
      .toUpperCase();
    if (/^WORK\s*UPDATES?$/.test(t) || /^(UPDATE|UPDATES)$/.test(t) || /^WORK\s*STATUS$/.test(t)) {
      return true;
    }
  }
  return false;
...
export function parseWeeklyPlanMatrix(matrix) {
  ...
  for (let r = dataStart; r < matrix.length; r += 1) {
    ...
    for (const day of dayCols) {
      if (halfLayout) {
        if (!isWorkUpdateHeaderCol(matrix, day.firstCol)) {
          const first = cellText(row[day.firstCol]);
          pushHalfTask(tasks, { ymd: day.ymd, taskName, srNo, half: 1, content: first });
        }
        if (day.secondCol != null && !isWorkUpdateHeaderCol(matrix, day.secondCol)) {
          const second = cellText(row[day.secondCol]);
          pushHalfTask(tasks, { ymd: day.ymd, taskName, srNo, half: 2, content: second });
        }
      } else {
        let col = day.firstCol;
        if (col != null && isWorkUpdateHeaderCol(matrix, col)) {
          col = col > 1 && !isWorkUpdateHeaderCol(matrix, col - 1) ? col - 1 : null;
        }
        if (col == null) continue;
        const plan = cellText(row[col]);
