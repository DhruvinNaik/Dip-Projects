...
export function WeeklyPlanAttachmentPreview({
  ...
  const loadTasks = useCallback(async () => {
    ...
    // Older ingests may have stored the uploaded file name instead of the
    // attachment key. Load the EM tasks once without the filter so they can
    // still be matched to the visible cells and completed from the portal.
  ...
  const setTaskStatus = async (taskList, nextStatus, busyKey) => {
    ...
    try {
      const results = await Promise.all(
        targets.map((task) =>
          api(`/ea-meeting/tasks/${task.id}/status`, {
            method: "PATCH",
