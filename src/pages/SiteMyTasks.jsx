...
import {
  currentPortalUser,
  ingestWeeklyPlanTasks,
  listTasksForEa,
  loadMyPlanTasks,
  setWeeklyPlanTaskStatus,
} from "../lib/eaMeeting";
...
async function ingestPlanAttachment(eaId, sourceFile, fileUrl, fileName) {
  ...
  const existing = await listTasksForEa(eaId, sourceFile).catch(() => []);
  if (Array.isArray(existing) && existing.length) {
    return { inserted: 0, already: existing.length };
  }
...
export default function SiteMyTasks() {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const data = await loadMyPlanTasks(currentPortalUser());
      applyPayload(data);
    } catch (err) {
  ...
  const syncFromPlans = useCallback(async () => {
    ...
    try {
      ...
      const existing = await loadMyPlanTasks(currentPortalUser());
      const plans = Array.isArray(existing?.uploads) ? existing.uploads : [];
  ...
  const toggleStatus = async (task) => {
    ...
    try {
      const result = await setWeeklyPlanTaskStatus(task.id, wanted);
      if (result?.task?.id) {
