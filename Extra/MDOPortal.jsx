...
// DPR sheet: only engineers with ≥1 DPR (evening) in the selected period.
async function fetchDprSheet(sites, from, to) {
  ...
  uniqueSites.forEach((site) => {
    ...
    submittedEngs.forEach((entry) => {
      // Must have at least one DONE day in range — never list all-PEND names.
      if (!entry.dates?.size) return;
      const days = dates.map((d) => (entry.dates.has(d) ? "DONE" : "PEND"));
      if (!days.includes("DONE")) return;
